...
<script>
    angular.module('myApp', [])
        .controller('Controller', function($scope, $http) {
            ...
            $scope.executaQuery = function(sql = $scope.sql) {
                if (sql != '') {
                    $scope.resultados = [];
                    $scope.erro = '';
                    $http.post("../Controller/DAOEditorSQL.Class.php?funcao=executaQueryController", sql).then(function(response) {
            ...
            document.addEventListener('keydown', function(event) {
                if (event.ctrlKey && event.key === 'e') {
                    event.preventDefault(); // Evita o comportamento padrão do navegador (abrir a busca)
                    let selectedText = window.getSelection().toString(); // Obtém o texto selecionado
                    selectedText;
                    $scope.executaQuery(selectedText);
                    // Você pode fazer o que quiser com o texto selecionado aqui
                }
            });
